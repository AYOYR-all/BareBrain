...
MimiClawは小さなESP32-S3ボードをパーソナルAIアシスタントに変えます。USB電源に接続し、WiFiにつなげて、Feishu/Larkから話しかけるだけ — どんなタスクも処理し、ローカルメモリで時間とともに成長します — すべて親指サイズのチップ上で。

## MimiClawの特徴

- **超小型** — Linux不要、Node.js不要、無駄なし — 純粋なCのみ
- **便利** — Feishu/Larkでメッセージを送るだけ、あとはお任せ
...
Feishu/Larkでメッセージを送ると、ESP32-S3がWiFi経由で受信し、エージェントループに送ります — LLMが思考し、ツールを呼び出し、メモリを読み取り — 返答を送り返します。**Anthropic (Claude)** と **OpenAI (GPT)** の両方をサポートし、実行時に切り替え可能です。すべてが$5のチップ上で動作し、データはすべてローカルのFlashに保存されます。
...
- **USB Type-Cケーブル**
- **Feishu/Lark の App ID と App Secret** — [Feishu Open Platform](https://open.feishu.cn/) または [Lark Developer](https://open.larksuite.com/) でカスタムアプリを作成
...
#define MIMI_SECRET_BASE_URL        ""              // 任意：https://openrouter.ai/api/v1
...
mimi> set_feishu_creds cli_xxx secret_xxx   # Feishu/Lark 認証情報を変更
mimi> feishu_send ou_xxx "hello"            # Feishu テストメッセージを送信
...
mimi> set_base_url https://openrouter.ai/api/v1  # 任意：OpenAI互換ゲートウェイのAPIルート
...
`MIMI_SECRET_BASE_URL` を空のままにすると公式エンドポイントを使います。OpenAI互換ゲートウェイを使う場合は `provider=openai` のまま、`https://openrouter.ai/api/v1` のような API ルートを指定してください。
