...
#define MIMI_SECRET_BASE_URL        ""              // 可选：https://openrouter.ai/api/v1
...
在受限网络环境下，访问飞书开放平台或 Anthropic API 可能需要代理。MimiClaw 内置 HTTP CONNECT 隧道支持。
...
mimi> set_feishu_creds cli_xxx secret_xxx   # 换飞书/Lark 凭据
mimi> feishu_send ou_xxx "hello"            # 发送飞书测试消息
mimi> set_relay wss://relay.example.com/ws demo-board board-secret
mimi> clear_relay                           # 清除 relay 配置
...
mimi> set_base_url https://openrouter.ai/api/v1  # 可选：OpenAI 兼容网关根地址
mimi> set_proxy 192.168.1.83 7897  # 设置代理
mimi> clear_proxy                  # 清除代理
...
如果 `MIMI_SECRET_BASE_URL` 留空，就继续使用官方端点。要接 OpenAI 兼容网关时，保持 `provider=openai`，再把它设成类似 `https://openrouter.ai/api/v1` 这样的 API 根地址即可。
