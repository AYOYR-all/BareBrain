...
当前固件仅支持 `SDMMC / SDIO 4-bit`，不需要手动输入挂载命令。设备启动时会先挂载 `/spiffs`，然后尝试挂载 `/sdcard`。如果 SD 挂载成功，记忆、会话和文档会优先写入 SD；如果失败，会明确打印日志并继续使用 SPIFFS。

当前 `SDMMC / SDIO 4-bit` 固件下，TF 模块按下面方式接线：
