...
- 一台公网 relay 服务器，如果你想在局域网外远程连到这块板子
...
brn> memory_read              # 看看它记住了什么
brn> memory_write "内容"       # 写入 MEMORY.md
...
BareBrain 把核心状态和长期数据都存为纯文本文件，可以直接读取和编辑：
...
- `/sdcard` 在挂载成功时优先保存增长数据：`memory/`、`sessions/`、`docs/`
- 如果没有插 SD 卡，或挂载失败，系统会继续使用 SPIFFS，不影响正常聊天和启动

例如：

- `MEMORY.md` 挂载成功时优先位于 `/sdcard/memory/MEMORY.md`
- 聊天记录优先位于 `/sdcard/sessions/<chat_id>.jsonl`
- `cron.json` 和 `HEARTBEAT.md` 仍固定保留在 `/spiffs`

## SD 卡存储

当前固件默认启用 `SDMMC / SDIO 4-bit` 模式，不需要手动输入挂载命令。设备启动时会先挂载 `/spiffs`，然后尝试挂载 `/sdcard`。如果 SD 挂载成功，记忆、会话和文档会优先写入 SD；如果失败，会明确打印日志并继续使用 SPIFFS。

你这块双模 TF 模块按当前默认固件应这样接：
...
- `VCC` 必须接 `3.3V`，不要接 `5V`
- 当前默认按 `SDIO` 名称理解丝印，不按 `SPI` 名称理解
- `CS / D03` 在 `SDIO 4-bit` 下实际是 `D3`，必须接
- 如果你后面想改成 `SPI` 模式，需要修改 `main/brn_config.h` 中的 `BRN_SD_MODE`
...
如果看到 `SD mounted: no`，设备仍会继续工作，但增长数据会回到 SPIFFS。
...
- **SD 卡存储** — 启动时自动尝试挂载，成功后记忆、会话、文档优先写入 `/sdcard`
