...
Configuration guides for BareBrain's external service integrations.

Related local setup guide:

- [Wi-Fi AP Onboarding Guide](../WIFI_ONBOARDING_AP.md) — configure firmware builds that expose the local `BareBrain-XXXX` onboarding/admin access point
...
BareBrain integrates with external services to extend its capabilities. Each guide below walks through obtaining API credentials, configuring BareBrain (build-time or runtime), and verifying the integration.
