...
This guide documents the Wi-Fi onboarding flow for firmware builds that include the BareBrain onboarding portal.

## What It Does

When onboarding is enabled, BareBrain can expose a local Wi-Fi access point such as `BareBrain-XXXX` and serve a configuration page at `http://192.168.4.1`.
...
- a phone or laptop that can join the temporary `BareBrain-XXXX` network

## First-Time Setup

1. Power on the device.
2. Wait for the onboarding hotspot `BareBrain-XXXX` to appear.
...
If the firmware keeps the admin AP online after normal Wi-Fi connection, you can reconnect to `BareBrain-XXXX` later and open `http://192.168.4.1` again.
...
1. saved NVS values
2. build-time defaults from `main/brn_secrets.h`

That means:

- if a field exists in NVS, it overrides the build-time default
- if you clear a saved field and reboot, the device falls back to `main/brn_secrets.h`
...
### No `BareBrain-XXXX` hotspot appears
...
### The build-time provider/model/base URL do not show up

If the device already has `model`, `provider`, or `base_url` saved in NVS, the saved values win over `main/brn_secrets.h`.
