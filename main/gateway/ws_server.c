...
static ws_client_t s_clients[BRN_WS_MAX_CLIENTS];

static ws_client_t *find_client_by_fd(int fd)
{
    for (int i = 0; i < BRN_WS_MAX_CLIENTS; i++) {
...
esp_err_t ws_server_start(void)
{
    ...
    config.server_port = BRN_WS_PORT;
    config.ctrl_port = BRN_WS_PORT + 1;
    config.max_open_sockets = BRN_WS_MAX_CLIENTS;
    ...
    ESP_LOGI(TAG, "WebSocket server started on port %d", BRN_WS_PORT);
    return ESP_OK;
}
