...
static char s_provider[LLM_PROVIDER_MAX_LEN] = BRN_LLM_PROVIDER_DEFAULT;
...
#if BRN_LLM_LOG_VERBOSE_PAYLOAD
...
#else
    if (BRN_LLM_LOG_PREVIEW_BYTES > 0) {
        size_t shown = total > BRN_LLM_LOG_PREVIEW_BYTES ? BRN_LLM_LOG_PREVIEW_BYTES : total;
        char preview[BRN_LLM_LOG_PREVIEW_BYTES + 1];
...
static esp_err_t llm_resolve_endpoint(llm_endpoint_t *endpoint)
{
    esp_err_t err = llm_endpoint_build(s_provider, s_base_url, endpoint);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Invalid LLM endpoint (provider=%s, base_url=%s): %s",
                 s_provider,
...
static void build_host_header(const llm_endpoint_t *endpoint, char *host_header, size_t host_header_size)
{
    if (endpoint->port == 443) {
        safe_copy(host_header, host_header_size, endpoint->host);
        return;
    }
    snprintf(host_header, host_header_size, "%s:%u", endpoint->host, endpoint->port);
}
...
esp_err_t llm_proxy_init(void)
{
    ...
    if (BRN_SECRET_API_KEY[0] != '\0') {
        safe_copy(s_api_key, sizeof(s_api_key), BRN_SECRET_API_KEY);
    }
    if (BRN_SECRET_MODEL[0] != '\0') {
        safe_copy(s_model, sizeof(s_model), BRN_SECRET_MODEL);
    }
    if (BRN_SECRET_MODEL_PROVIDER[0] != '\0') {
        safe_copy(s_provider, sizeof(s_provider), BRN_SECRET_MODEL_PROVIDER);
    }
    if (BRN_SECRET_BASE_URL[0] != '\0') {
        safe_copy(s_base_url, sizeof(s_base_url), BRN_SECRET_BASE_URL);
    }

    /* NVS overrides take highest priority (set via CLI) */
    nvs_handle_t nvs;
    if (nvs_open(BRN_NVS_LLM, NVS_READONLY, &nvs) == ESP_OK) {
        char tmp[LLM_API_KEY_MAX_LEN] = {0};
        size_t len = sizeof(tmp);
        if (nvs_get_str(nvs, BRN_NVS_KEY_API_KEY, tmp, &len) == ESP_OK && tmp[0]) {
            safe_copy(s_api_key, sizeof(s_api_key), tmp);
        }
        char model_tmp[LLM_MODEL_MAX_LEN] = {0};
        len = sizeof(model_tmp);
        if (nvs_get_str(nvs, BRN_NVS_KEY_MODEL, model_tmp, &len) == ESP_OK && model_tmp[0]) {
            safe_copy(s_model, sizeof(s_model), model_tmp);
        }
        char provider_tmp[LLM_PROVIDER_MAX_LEN] = {0};
        len = sizeof(provider_tmp);
        if (nvs_get_str(nvs, BRN_NVS_KEY_PROVIDER, provider_tmp, &len) == ESP_OK && provider_tmp[0]) {
            safe_copy(s_provider, sizeof(s_provider), provider_tmp);
        }
        char base_url_tmp[LLM_BASE_URL_MAX_LEN] = {0};
        len = sizeof(base_url_tmp);
        if (nvs_get_str(nvs, BRN_NVS_KEY_BASE_URL, base_url_tmp, &len) == ESP_OK && base_url_tmp[0]) {
...
static esp_err_t llm_http_direct(const char *post_data, resp_buf_t *rb, int *out_status)
{
    ...
    esp_err_t err = llm_resolve_endpoint(&endpoint);
    if (err != ESP_OK) {
        return err;
    }
...
static esp_err_t llm_http_via_proxy(const char *post_data, resp_buf_t *rb, int *out_status)
{
    ...
    char header[1024];
    char host_header[LLM_ENDPOINT_HOST_MAX_LEN + 8];
    int hlen = 0;
    build_host_header(&endpoint, host_header, sizeof(host_header));
    if (provider_is_openai()) {
        hlen = snprintf(header, sizeof(header),
            ...
            "Content-Length: %d\r\n"
            "Connection: close\r\n\r\n",
            endpoint.path, host_header, s_api_key, body_len);
    } else {
        hlen = snprintf(header, sizeof(header),
            "POST %s HTTP/1.1\r\n"
            "Host: %s\r\n"
            ...
            "Content-Length: %d\r\n"
            "Connection: close\r\n\r\n",
            endpoint.path, host_header, s_api_key, BRN_LLM_API_VERSION, body_len);
    }
                         ...
                         llm_response_t *resp)
{
    ...
    if (provider_is_openai()) {
        cJSON_AddNumberToObject(body, "max_completion_tokens", BRN_LLM_MAX_TOKENS);
    } else {
        cJSON_AddNumberToObject(body, "max_tokens", BRN_LLM_MAX_TOKENS);
    }
    ...
    /* HTTP call */
    resp_buf_t rb;
    if (resp_buf_init(&rb, BRN_LLM_STREAM_BUF_SIZE) != ESP_OK) {
        free(post_data);
            ...
            if (message) {
                ...
                if (tool_calls && cJSON_IsArray(tool_calls)) {
                    ...
                    cJSON_ArrayForEach(tc, tool_calls) {
                        if (resp->call_count >= BRN_MAX_TOOL_CALLS) break;
...
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(BRN_NVS_LLM, NVS_READWRITE, &nvs));
...
esp_err_t llm_set_model(const char *model)
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(BRN_NVS_LLM, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, BRN_NVS_KEY_MODEL, model));
    ESP_ERROR_CHECK(nvs_commit(nvs));
    nvs_close(nvs);
...
esp_err_t llm_set_provider(const char *provider)
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(BRN_NVS_LLM, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, BRN_NVS_KEY_PROVIDER, provider));
    ESP_ERROR_CHECK(nvs_commit(nvs));
    nvs_close(nvs);
...
esp_err_t llm_set_base_url(const char *base_url)
{
    ...
    if (base_url && base_url[0] != '\0') {
        esp_err_t err = llm_endpoint_build(s_provider, base_url, &endpoint);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Rejected invalid base URL: %s", base_url);
            return err;
        }
    }
    ...
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(BRN_NVS_LLM, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, BRN_NVS_KEY_BASE_URL, base_url));
    ESP_ERROR_CHECK(nvs_commit(nvs));
    nvs_close(nvs);
