...
{
    s_inbound_queue = xQueueCreate(BRN_BUS_QUEUE_LEN, sizeof(brn_msg_t));
    s_outbound_queue = xQueueCreate(BRN_BUS_QUEUE_LEN, sizeof(brn_msg_t));
    ...
    ESP_LOGI(TAG, "Message bus initialized (queue depth %d)", BRN_BUS_QUEUE_LEN);
    return ESP_OK;
}

esp_err_t message_bus_push_inbound(const brn_msg_t *msg)
{
...
esp_err_t message_bus_pop_inbound(brn_msg_t *msg, uint32_t timeout_ms)
{
...
esp_err_t message_bus_push_outbound(const brn_msg_t *msg)
{
...
esp_err_t message_bus_pop_outbound(brn_msg_t *msg, uint32_t timeout_ms)
{
    TickType_t ticks = (timeout_ms == UINT32_MAX) ? portMAX_DELAY : pdMS_TO_TICKS(timeout_ms);
